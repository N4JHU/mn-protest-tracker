...
REPO_OWNER = "N4JHU" # <--- YOUR GITHUB USERNAME
REPO_NAME = "mn-protest-tracker"

# --- SOURCES ---
ARCGIS_METADATA_URL = "https://www.arcgis.com/sharing/rest/content/items/081587d29d944a89ad189b1633e509e4?f=json"
NEWS_QUERY = "(protest OR riot OR march OR police OR crash OR gunfire OR standoff OR ICE OR immigration OR whipple OR agents OR deportation) AND (site:startribune.com OR site:wcco.com OR site:kstp.com OR site:kare11.com OR site:mprnews.org OR site:bringmethenews.com OR site:dps.mn.gov OR site:minneapolismn.gov OR site:stpaul.gov OR site:aclu-mn.org OR site:cuapb.org) when:12h"
...
THREAT_KEYWORDS = ["riot", "tear gas", "standoff", "looting", "shots fired", "shuts down", "blocking", "ice agents", "immigration", "whipple", "federal building", "deportation", "breaking news"]
VIGIL_KEYWORDS = ["vigil", "gathering", "march", "rally", "memorial", "protest"]

LOCATIONS = {
    "capitol": (44.9543, -93.1022), "gov": (44.9543, -93.1022), "uptown": (44.9497, -93.2933),
    "downtown": (44.9765, -93.2761), "city hall": (44.9761, -93.2636), "3rd precinct": (44.9481, -93.2356),
    "1st precinct": (44.9775, -93.2650), "hennepin": (44.9780, -93.2630), "94": (44.9650, -93.2750),
    "35w": (44.9740, -93.2540), "fort snelling": (44.8940, -93.1760), "whipple": (44.8940, -93.1760),
            ...
            if lat and lng:
                ...
                if "test" not in title.lower():
                    events.append({
                        ...
                        "title": title, "lat": float(lat), "lng": float(lng), "type": "road_closure",
                        "desc": attrs.get("EventDescription", "See 511mn.org"), "timestamp": get_utc_now()
                    })
        return events
    except Exception as e: print(f"!! Road Scraper Error: {e}"); return []
    ...
    try:
        ...
        for item in root.findall('.//item'):
            ...
            for landmark, coords in LOCATIONS.items():
                if landmark in title.lower(): lat, lng = coords; break
            lat += random.uniform(-0.02, 0.02); lng += random.uniform(-0.02, 0.02)
            events.append({
                "id": f"news-{hash(title)}", "title": f"INTEL: {title[:60]}...", "lat": lat, "lng": lng, "type": "protest",
                "desc": f"<b>SOURCE: {source_tag}</b><br>{title}<br><br><a href='{link}' target='_blank' style='color:#ff5555'>OPEN SOURCE LINK</a>", "timestamp": get_utc_now()
            })
            ...
            if resp.status_code == 200:
                ...
                for post in posts:
                    ...
                    if is_threat or is_vigil:
                        ...
                        events.append({
                            "id": f"reddit-{data.get('id')}", "title": f"REDDIT: {title[:50]}...", "lat": lat, "lng": lng, "type": "protest",
                            "desc": f"<b>TAG: {tag}</b><br>u/{data.get('author')}: {title}<br><br><a href='https://reddit.com{data.get('permalink')}' target='_blank' style='color:#ff5555'>VIEW THREAD</a>", "timestamp": get_utc_now()
                        })
            time.sleep(1)
    except Exception as e: print(f"!! Reddit Error: {e}")
    return events

# --- MODULE 3: USER FIELD REPORTS (FROM GITHUB ISSUES) ---
def get_user_reports():
    print("--- Checking for Field Reports in Issues ---")
    events = []
    # Public API to read issues (Rate limit 60/hr is plenty for 5m interval)
    api_url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/issues?labels=field-report&state=open"
    try:
            ...
            for issue in issues:
                ...
                if match:
                    try:
                        ...
                        if "lat" in report_data and "lng" in report_data and "title" in report_data:
                            # Force type to 'report' for purple color
                            report_data["type"] = "report"
                            # Ensure unique ID
                            report_data["id"] = f"rept-{issue['number']}"
                            events.append(report_data)
                    except: continue
        print(f"--- SUCCESS: Found {len(events)} user reports.")
        return events
    except Exception as e:
        print(f"!! Issue Scraper Error: {e}")
        return []
...
def main():
    ...
    new_events.extend(get_user_reports()) # <--- NEW MODULE

    event_db = {e["id"]: e for e in existing_events} 
    for event in new_events: event_db[event["id"]] = event
    ...
    with open(OUTPUT_FILE, "w") as f:
        json.dump({"last_updated": get_utc_now(), "features": final_features}, f, indent=2)
